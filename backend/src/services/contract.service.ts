...
import { generateNextContractNumber } from '../utils/contractNumber.utils';
import { AuditService } from './audit.service';
import { Request } from 'express';
import { validateAndNormalizeContractValues } from '../utils/contractValue.utils';
...
export class ContractService {
  ...
  static async createContract(data: any, req?: Request) {
    const contractNumber = await generateNextContractNumber();
    const template = await prisma.contractTemplate.findFirst({
      where: { contractTypeId: data.contractTypeId, isDefault: true },
      include: { fields: true },
    });
    const normalizedValues = validateAndNormalizeContractValues(data.values, template?.fields || []);
  ...
  static async updateContract(id: string, data: any, req?: Request) {
    ...
    const normalizedValues = data.values !== undefined
      ? validateAndNormalizeContractValues(data.values, template?.fields || [])
      : null;
  ...
  static async updateStatus(id: string, status: string, req?: Request) {
    const contract = await prisma.contract.findUnique({ where: { id } });
    if (!contract || contract.deletedAt) throw new Error('العقد غير موجود');
  ...
  static async archiveContract(id: string, req?: Request) {
    const contract = await prisma.contract.findUnique({ where: { id } });
    if (!contract || contract.deletedAt) throw new Error('العقد غير موجود');
  ...
  static async restoreContract(id: string, req?: Request) {
    const contract = await prisma.contract.findUnique({ where: { id } });
  ...
  static async softDeleteContract(id: string, req?: Request) {
    const contract = await prisma.contract.findUnique({ where: { id } });
    if (!contract) throw new Error('العقد غير موجود');
