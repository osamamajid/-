...
interface ContractFieldDefinition {
  fieldKey: string;
  label: string;
  fieldType: string;
  isRequired: boolean;
...
const isNameField = (field: ContractFieldDefinition) =>
  NAME_KEY_PATTERN.test(field.fieldKey) || NAME_LABEL_PATTERN.test(field.label);

const isEmpty = (value: unknown) => value === undefined || value === null || value === '';

export const validateAndNormalizeContractValues = (
  values: unknown,
  ...
  for (const field of fields) {
    ...
    if (isEmpty(value)) {
      if (field.isRequired) throw new Error(`حقل "${field.label}" مطلوب`);
      continue;
    }
    ...
    switch (field.fieldType) {
      case 'TEXT':
      case 'TEXTAREA': {
        if (typeof value !== 'string') throw new Error(`حقل "${field.label}" يجب أن يكون نصًا`);
      ...
      case 'DATE':
        ...
        break;
      case 'SELECT': {
        if (typeof value !== 'string') throw new Error(`يجب اختيار قيمة صحيحة لحقل "${field.label}"`);
      ...
      case 'CHECKBOX':
        if (typeof value !== 'boolean') throw new Error(`حقل "${field.label}" يجب أن يكون نعم أو لا`);
